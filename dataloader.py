...
# The code is then modified by the Shangmin Guo, 2024.
"""
...
class Example:
    """
    Class for an example in a preference or SFT dataset. If you want each 
    prompt to be uniquely associated with an Example instance, save it in a 
    dict.
    """
    prompt: str = ''                          # prompt for the generated texts
    generations: List[str] = field(default_factory=list) # list of generations
    sft_index: int = -1                 # idx of generations as target for SFT
    # score for each generation
    scores: List[float] = field(default_factory=list)
    # for binary feedback data::
    # indices in responses, where i > j in pair (i,j) is a preference
    pairs: List[Tuple[int, int]] = field(default_factory=list)
     # for unary feedback data: whether the generation at the corresponding
    # index in self.generations is desirable 
...
def get_alpacaeval(
    ...
    """
    Load the AlpacaEval dataset (for evaluation only) and convert it into to a 
    Dataset.
    ...
    rank0_print(
        f'Loading AlpacaEval dataset ({split} split) from Huggingface...'
    )
    ...
    for row in dataset:
        prompt = human_prefix + row['instruction'] \
                 + human_suffix + assistant_prefix
        data[prompt].prompt = prompt
        data[prompt].generations.append(row['output'] + assistant_suffix)
        data[prompt].dataset_name = row['dataset']
        # keep original prompt so that it can be dumped into a JSON file before
        # running the alpacaeval command
...
class SimpleKTODataLoader(DataLoader):
    """
    Legacy Dataloader for the original variant of KTO that presumes access to 
    even number of desirable and undesirable examples in each microbatch.

    Each batch contains half (x, desired output y) and half (x, undesired 
    output y), where no x should appear twice because of shuffling. The 
    desirable and undesirable examples are interleaved in the batch (e.g.,
    ...
    def __iter__(self):
        ...
        while True:
            ...
            for example, generation, status in flat_data:
                ...
                # only flush queues when you can get an even number of chosen
                # and rejected examples weave together chosen and rejected 
                # examples one after the other to prevent per-device microbatch
                # from being all chosen or all rejected
                if len(chosen_example_queue) >= quota and \
                    len(rejected_example_queue) >= quota:
...
class PairwisePreferenceDataLoader(DataLoader):
    """
...
class PointwisePreferenceDataLoader(DataLoader):
    """
